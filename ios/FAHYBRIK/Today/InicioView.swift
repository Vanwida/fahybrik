...
// Inicio tab root — "CAMINO A LA CARRERA". The home is a VERDICT toward the race,
// not a calendar: it shows STATE + trajectory + one action, never the future /
// week / session-selection (that's the Plan tab) and never duplicates the Plan's
// "Foco de la semana".
//
// Layout (top → bottom):
//   header  → greeting
//   1. RACE ANCHOR — "Camino a {target race} · N días" + "{fase} · semana N de M"
//      + the goal time + a block-position bar. The peak/objective race, not the
//      nearest tune-up. (Empty → invite to pick a target.)
//   2. ¿CÓMO LLEGAS HOY? — readiness clearance: 0–100 score + a plain read + a
//      mini breakdown of the signals feeding it (check-in / HRV / sueño / FC reposo).
//   3. TU PROGRESO — proof you're advancing: current 1RM + the athlete's most
//      relevant REAL biometric trend (HRV / VO₂max / FC reposo / sueño). Hidden
//      when there is genuinely nothing to show.
//   4. ONE ACTION — start TODAY's session (a single action card, not a menu).
...
// projected finish time (no model exists yet), no weakest-station (those tables
// were dropped), no week strip / session picker, no standalone focus line.
//
...
struct InicioView: View {
    ...
    // "Tu progreso" inputs.
    private var biometricPrimary: BiometricMetricSeries? { store.biometricTrend.value?.primary }
    ...
    // MARK: - 1 · Race anchor ("Camino a la carrera")
    //
    // The emotional lead: where the athlete is going (the OBJECTIVE race), how far
    // out, where they are in the plan ("{fase} · semana N de M") and the goal time.
    // A subtle position bar shows progress through the current microciclo's weeks —
    // that's POSITION in the block, not a calendar of upcoming sessions.
    ...
    // MARK: - 3 · Tu progreso (proof you're advancing)
    //
    // Identity / proof: the athlete's current 1RM (no "tendencia" label unless there
    // are ≥2 versions to compare) plus their most relevant REAL biometric trend
    // (HRV / VO₂max / FC reposo / sueño — whichever has recent data). Hidden when
    // there is genuinely nothing to show; never a fabricated metric.

    @ViewBuilder
    private var progressCard: some View {
        let lift = topLift
        let metric = biometricPrimary
        if lift != nil || metric != nil {
            CardSurface(padding: Theme.Spacing.l) {
                VStack(alignment: .leading, spacing: 14) {
                    LabelText(text: "Tu progreso")
                    if let lift { liftRow(lift) }
                    if lift != nil && metric != nil { Hairline() }
                    if let metric { trendRow(metric) }
                }
            }
            .accessibilityElement(children: .contain)
        } else if store.strengthMaxes.hasLoaded && store.biometricTrend.hasLoaded {
            // Truly nothing yet — honest, quiet empty state.
            CardSurface(padding: Theme.Spacing.l) {
                VStack(alignment: .leading, spacing: 4) {
                    LabelText(text: "Tu progreso")
                    Text("Registra entrenos para ver tu progreso")
    ...
    /// Current 1RM for the headline lift. Shows an evolution delta ONLY when there
    /// are ≥2 versions — a single test has no trend to claim.
    private func liftRow(_ lift: StrengthMaxProfile) -> some View {
        HStack(alignment: .center, spacing: 10) {
            ...
            Spacer(minLength: 8)
            if let evo = liftEvolution(lift) {
                liftSparkline(evo)
            }
            VStack(alignment: .trailing, spacing: 2) {
                Text(lift.oneRmLabel)
                    .font(.system(size: 20, weight: .heavy, design: .default).italic().monospacedDigit())
                    .foregroundStyle(Theme.Color.accentText)
                    .lineLimit(1)
                    .minimumScaleFactor(0.7)
                if let evo = liftEvolution(lift) {
                    liftDeltaBadge(evo.current - evo.previous)
                }
            }
        }
        .accessibilityElement(children: .ignore)
        .accessibilityLabel(liftAxLabel(lift))
    }

    private struct LiftEvolution {
        let values: [Double]
        let previous: Double
        let current: Double
    }

    /// Chronological 1RM series across versions; nil when there's only one (so the
    /// row shows the number with NO trend, per spec).
    private func liftEvolution(_ m: StrengthMaxProfile) -> LiftEvolution? {
        ...
        return LiftEvolution(values: series, previous: series[series.count - 2], current: series[series.count - 1])
    }

    private func liftSparkline(_ evo: LiftEvolution) -> some View {
        TrendSparkline(values: evo.values, color: Theme.Color.accentText)
            .frame(width: 56, height: 24)
            .accessibilityHidden(true)
    }

    private func liftDeltaBadge(_ delta: Double) -> some View {
    ...
    private func liftAxLabel(_ lift: StrengthMaxProfile) -> String {
        var label = "\(lift.exerciseLabel), \(lift.oneRmLabel)"
        if let evo = liftEvolution(lift) {
            let delta = Int((evo.current - evo.previous).rounded())
            label += delta == 0 ? "" : ", \(delta > 0 ? "sube" : "baja") \(abs(delta)) kilos"
        }
        return label
    }

    /// The athlete's most relevant biometric trend (engine/recovery) — a sparkline,
    /// the latest value, and an honest direction badge.
    private func trendRow(_ m: BiometricMetricSeries) -> some View {
        HStack(alignment: .center, spacing: 10) {
            VStack(alignment: .leading, spacing: 2) {
                LabelText(text: m.label, size: 10)
                Text(metricValueLabel(m))
                    .scaledFont(15, weight: .semibold, relativeTo: .subheadline)
                    .foregroundStyle(Theme.Color.foreground)
            }
            Spacer(minLength: 8)
            TrendSparkline(values: m.points.map { $0.value }, color: Theme.Color.foreground.opacity(0.55))
                .frame(width: 64, height: 24)
                .accessibilityHidden(true)
            trendBadge(m)
        }
        .accessibilityElement(children: .ignore)
        .accessibilityLabel(trendAxLabel(m))
    }

    /// "43" / "48 ms" / "7.5 h" — the latest real reading.
    private func metricValueLabel(_ m: BiometricMetricSeries) -> String {
        let v = m.latest
    ...
    @ViewBuilder
    private func trendBadge(_ m: BiometricMetricSeries) -> some View {
        if m.isMoving {
            let improving = m.isImproving
            HStack(spacing: 3) {
                Image(systemName: m.direction == "up" ? "arrow.up.right" : "arrow.down.right")
                    .font(.system(size: 10, weight: .bold))
                Text(improving ? "Mejora" : "Estable")
                    .scaledFont(10, weight: .semibold, relativeTo: .caption2)
            }
            .foregroundStyle(improving ? Theme.Color.ok : Theme.Color.muted)
        } else {
            Text("→")
    ...
    private func trendAxLabel(_ m: BiometricMetricSeries) -> String {
        let dir = m.isMoving ? (m.isImproving ? ", mejorando" : ", estable") : ""
        return "\(m.label), \(metricValueLabel(m))\(dir)"
    }

    // MARK: - 4 · One action (today's session)
