...
final class PushAppDelegate: NSObject, UIApplicationDelegate, UNUserNotificationCenterDelegate {
    ...
    /// so the queries exist before HealthKit delivers a background update.
    /// Credentials are the existing Keychain bearer + persisted athlete id.
    /// `start()` is idempotent; AppRoot / Perfil / día-1 still call it after login.
