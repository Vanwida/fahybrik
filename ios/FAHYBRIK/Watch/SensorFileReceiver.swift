...
final class SensorFileReceiver {
    ...
    private init() {
        ...
        inboxDir = base.appendingPathComponent("sensor-captures", isDirectory: true)
        try? FileManager.default.createDirectory(at: inboxDir, withIntermediateDirectories: true)
    }

    /// Called from WCSessionDelegate when a file arrives from the wrist.
    func didReceive(file: WCSessionFile) {
        let meta = file.metadata ?? [:]
        let localId = (meta["execution_local_id"] as? String) ?? UUID().uuidString
        let dest = inboxDir.appendingPathComponent("\(localId).fhsc")
        ...
        do {
            ...
            enqueuePending(path: dest.path, meta: meta)
        } catch {
            // Leave the system temp file; next session can retry nothing — log only.
            #if DEBUG
            print("SensorFileReceiver copy failed: \(error)")
            #endif
        }
    }
