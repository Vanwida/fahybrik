...
1. **Selector de PESO rápido en el builder libre** (Alex, entrenando): el −/+ para la carga es lentísimo. Sustituir por una **rueda** (el patrón nativo que ya usamos en Registrar carrera) con pasos de 2,5 kg — y probar al construirlo si una regla horizontal arrastrable con ticks hápticos se siente mejor («algo guay»); decidir en el dispositivo, no en papel. Aplica a todos los campos de kg del builder. **Y EN VIVO** (captura de Alex entrenando, IMG_2385): en la pantalla del entreno activo, poder tocar el peso de la SIGUIENTE serie y subirlo/bajarlo ahí mismo con la misma rueda — el modelo ya lo soporta (`execution_segments.weight_used_kg` registra el peso real por serie); es UI sobre datos que ya existen. La serie hecha conserva su peso real; las que vienen heredan el nuevo. **Y el DESCANSO avisa al acabar** (Alex, entrenando): vibración clara (háptico fuerte, patrón doble) cuando el timer de descanso llega a cero — es fácil distraerse; valorar aviso también a 10 s del final y el espejo en el watch (que vibre la muñeca, no solo el iPhone). `Haptics` ya existe; es engancharlo al cero del timer en iPhone y reloj. **Y el LANDSCAPE está roto** (captura IMG_2386): en las vistas que permiten horizontal la UI se descompone — botones escondidos bajo el borde. Decisión a tomar al construir: la raíz probablemente NO es arreglar cada layout sino hacer el landscape DELIBERADO — bloquear portrait en toda la app salvo las pocas pantallas que se ganan el horizontal (HUD de carrera/vídeo, si alguna), que es el estándar de las apps de gym. Auditar qué vistas lo permiten hoy por accidente. **Y el WATCH se congela en 0:00 con el iPhone en background** (captura IMG_2387): el espejo depende de ticks del teléfono, y al ir la app iOS a background dejan de llegar hasta volver a foreground. Raíz probable: el reloj NO debe depender del teléfono para CONTAR — conoce startedAt y puede llevar su propio reloj local; del iPhone solo necesitan viajar los CAMBIOS de estado (serie hecha, descanso, fin). Revisar también si la sesión de iPhone merece background runtime (HealthKit workout session) — mirárselo BIEN en todos los modos, no solo el del pantallazo (palabras de Alex). **Y los LIBRES: borrado roto + regla de fondo** (captura IMG_2389): Alex creó libres de prueba, los «borra» desde Hoy y reaparecen como pendientes del plan. REGLA que arregla bug y producto a la vez: un entreno libre NUNCA es una obligación — es el registro de algo que el atleta hizo. No existe en estado «pendiente»: o está hecho (y se pinta DIFERENCIADO del plan, badge «Libre» + tinte propio, dentro del tab Plan como pidió Alex) o el atleta lo borra DEL TODO (asignación + ejecución, es suyo). Auditar el camino de borrado actual: qué deja viva la asignación self-origin.

2. **Entreno libre con CALENTAMIENTO opcional** (petición de Alex entrenando en vivo): hoy el libre de fuerza arranca contando como si fuera la primera serie mientras él calienta. Diseño decidido: el builder libre gana una sección «Calentamiento» opcional (espejo de las fases del run-structure) — puede llevar ejercicios o ir VACÍA (solo la fase con su reloj, se salta cuando terminas). El plan generado pasa de un bloque «Libre» a dos («Calentamiento» + el principal) y el motor en vivo YA sabe recorrer bloques: la sesión empieza en calentamiento y la serie 1 es serie 1. Payload: items con parte (warmup|main) — revisar el contrato del free-save antes.
3. **Probar el fix del PM5 contra el remo físico** (705fe97: desconocido=sucio, terminate siempre salvo "wait to begin"). Escenario exacto: monitor con 100 m rodando → conectar → empezar benchmark → debe resetear a la pieza programada. iPhone al cable para instalar (build lista con la secuencia de conectar del ergo).
4. **Ranking del box — las pantallas**: el motor ya viaja en el GET de marcas (`box`: percentil, histograma 11 cubos, mediana, percentil de hace 90 días). Falta: tarjeta «En tu box» en el detalle de marca (iOS), sección «Tus marcas» en Analíticas (iOS), y la distribución CON nombres para Pablo (dashboard). Mockup aprobado: `docs/design/marcas-ranking-analiticas-mockup.html`.
5. **Editor: fuerza y metcon** con el patrón frase + fila abierta + defaults (el run ya está en prod). La tabla de ítems ya es readout; falta la edición inline en vez de la máquina de chips de `PrescriptionFields`, y la línea rápida de fuerza (la gramática ya parsea `5x5 @80% r2'30`; falta resolver ejercicio del catálogo por nombre).
6. La suite de tests de iOS sigue sin compilar por el test de chat de la otra sesión (ChatUploadResult sin Decodable) — suyo, en curso.
