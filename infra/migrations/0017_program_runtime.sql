-- Program runtime: month assignments, week adjustment proposals, monthly block proposals, readiness snapshots.

begin;

create type week_adjustment_status as enum ('pending', 'approved', 'rejected', 'superseded');

create table athlete_month_assignments (
...
create index athlete_month_assignments_athlete_idx
  on athlete_month_assignments (athlete_id, start_date desc);

create table week_adjustment_proposals (
  id                      bigint generated always as identity primary key,
  athlete_id              bigint not null references athletes(id) on delete cascade,
...
create unique index week_adjustment_proposals_pending_uniq
  on week_adjustment_proposals (athlete_id, week_start)
  where status = 'pending';

create index week_adjustment_proposals_athlete_idx
  on week_adjustment_proposals (athlete_id, week_start desc);

create type monthly_block_proposal_status as enum ('pending', 'approved', 'rejected');

create table monthly_block_proposals (
  id                    bigint generated always as identity primary key,
  athlete_id            bigint not null references athletes(id) on delete cascade,
  month_template_id     bigint not null references program_month_templates(id) on delete restrict,
...
create unique index monthly_block_proposals_pending_uniq
  on monthly_block_proposals (athlete_id)
  where status = 'pending';

create table athlete_daily_readiness_snapshots (
  id              bigint generated always as identity primary key,
  athlete_id      bigint not null references athletes(id) on delete cascade,
...
create index athlete_daily_readiness_athlete_day_idx
