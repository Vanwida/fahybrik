...
**El catálogo de fases está fuera** (migración 0148 aplicada en producción, con el código desplegado antes). Sobrevivió a las limpiezas de 0064/0068 porque **la columna viva no llevaba el prefijo del motor sino `target_block`**: una metodología se barre por semántica, nunca por su sigla. De paso caían avisos de transición en la cola de HOY, las anotaciones del gráfico de VO₂máx, un HTML público y **el propio `CLAUDE.md`**, que enseñaba un marco de periodización a todo agente que arrancara.
