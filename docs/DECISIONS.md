...
## 2026-08-29 · Correr en la muñeca son TRES PÁGINAS, y el vivo es el centro (card 105)

**Decidido (diseño; cero Swift, cero web, cero migraciones):** la carrera en el Apple Watch es **una sola interfaz de tres páginas** que se deslizan en horizontal, del mismo corte que la app de Entreno de Apple: **datos ← vivo → controles**. El vivo va en el CENTRO, no primero: es la página en la que se vive el entreno, y desde ella tanto las cifras como el pausar están a UN gesto — y volver al esfuerzo también. Ponerla primera dejaría el pausar a dos deslizamientos, corriendo. Los controles caen además donde la app ya tiene hoy su `PauseFinishPage` (página 2 del `TabView` de `LiveFlowView`), así que la memoria del gesto no se rompe. Mockups: `docs/mocks/reloj-correr*.png`.

**Qué es cada página, y de la definición sale todo lo demás:**

- **Vivo** — el sujeto es **lo que falta de la pieza que tienes delante, medido en la unidad en que esa pieza se mide**; si no falta nada medible (GPS sin fijar, pieza sin objetivo), el sujeto cae al reloj de esa pieza. Segundo nivel: el ritmo, y no hay tercero. **Cero controles pintados** — es el modo `ojeada` del kit (`kit-watch/modelo.ts`): corriendo no se toca, la pantalla sigue siendo un blanco entero para el gesto latente, y los 15 pt de la franja vuelven al número.
- **Datos** — la única página **sin sujeto**, y eso es su definición: tiempo, distancia, ritmo medio y pulso con su zona, **todo de la SESIÓN**, declarado en la banda de contexto. El vivo habla de la *pieza* y los datos de la *sesión*: dos páginas, dos alcances, cada uno escrito. Es la respuesta de raíz al fallo que ya se pagó una vez —«tiempo del tramo» enseñando lo que QUEDA junto a una distancia que enseñaba lo CUBIERTO, en la misma caja.
- **Controles** — pausar · nuevo tramo · terminar. La **única página con botones** de la interfaz, porque es la única a la que se llega habiendo decidido dejar de mirar y tocar (modo `mando`). Terminar va abajo, en rojo y confirmada (`¿Terminar y guardar?`, la pregunta que ya hace la app): un deslizamiento de más no puede acabar una carrera.

**«Nuevo tramo» es lo único que no existe hoy, y se define entero para no dejarlo a medias:** cierra la pieza que llevas medida y empieza a medir de cero, **sin tocar la prescripción**; produce un parcial con sus metros, su tiempo y su ritmo (el `LapRecord` del motor ya sabe guardarlo). Y **sólo se ofrece cuando los cortes son del atleta**: si el coach escribió la estructura, la frontera ya está escrita y la pieza se cierra al cruzar los metros o con el toque latente — ahí el botón no aparece. Es lo que evita dos nombres para el mismo acto: `Tramo hecho` avanza una frontera prescrita, `Nuevo tramo` crea una que nadie prescribió, y nunca conviven para la misma pieza.

**El mapa de bloques deja de ser una página del vivo.** Las tres páginas ocupan los dos deslizamientos que hay, y el mapa hoy vive en la página 0. Saltar de bloque es una **decisión**, así que baja a controles con las demás — donde ya está hoy `Siguiente bloque`. Cuando aparece ese cuarto botón la página rueda con la corona: es la única de la interfaz donde eso vale, porque a ella se llega parado.

**LA ZONA ES EL LIENZO, PLANA Y AL 45 % — y lo que la estaba apagando era el degradado, no el porcentaje.** Corrección de la primera versión de estas láminas, que bajó el tinte del 38 % del kit al 22 % «porque no leía como un tinte ambiente» (CONTRATO-UI §10.1). Diagnóstico equivocado: el `DEGRADADO` del kit va a **negro puro arriba y abajo** y deja el color vivo sólo en una franja estrecha del centro — justo donde va el numeral, que la tapa. Bajar el porcentaje encima de eso apagó el único sitio donde el color aún existía. Se arregla por la raíz: **relleno plano al 45 %** y del negro queda una **viñeta** (esquinas, donde la curva del bisel se come el lienzo, y las dos bandas de versales).

**El 45 % es un techo medido, no un gusto:** lo pone el **ámbar de la Z4**, porque el aro del bisel (`#FF8A4C`) tiene que mantener **3:1** contra el lienzo —es un elemento gráfico que hay que entender— y sobre ámbar al 45 % se queda en **3,08:1**; al 50 % ya es 2,67 y al 55 % 2,31, o sea que el aro desaparece. Con ese mismo 45 % el numeral blanco va de **7,18:1** (ámbar, el peor caso) a **12,25:1** (azul), y las cinco zonas se distinguen a un vistazo: Z1 `#3E3E40` · Z2 `#133173` · Z3 `#155E24` · Z4 `#73511D` · Z5 `#732323`.

**Todas las cifras de contraste de esta entrada están MEDIDAS en el navegador** —`color-mix` resuelto y leído por un lienzo 2D, luminancia relativa y razón de contraste— y no calculadas a mano. La primera versión de esta entrada traía tres de ellas mal y un hex equivocado (la Z3, con el 94 decimal escrito como `0x94`): calcular a ojo el contraste de un color que el navegador compone es exactamente el tipo de dato que se queda en un documento y luego nadie vuelve a comprobar.

**Y de un lienzo con color se derivan tres reglas que antes no hacían falta**, porque todo se había calibrado contra el negro:

1. **El gris secundario no puede ser un hex.** `#8A8A8E` (el `dim` de WatchTheme) se queda en **2,30:1** sobre la Z3 y en **2,09:1** sobre el ámbar: la unidad y las versales dejan de leerse. El cromo pasa a ser **blanco con alfa**, y el alfa también sale de medir — con 0,70 el peor caso (ámbar) da 4,47:1, por debajo del 4,5 que pide un texto pequeño, así que es **0,76**: 4,95:1 sobre el ámbar y 11,76:1 sobre negro, y sigue pesando menos que el dato (§4), que va en blanco puro.
2. **Mientras el lienzo lleve la zona, en esa pantalla no habla en color nada más.** El veredicto del ritmo es una **palabra**, no un color: un «en objetivo» verde sobre lienzo verde no se lee (3,9:1 y de la misma tinta), y sobre un lienzo ámbar el mismo verde diría dos cosas a la vez. Por lo mismo, el nombre de la zona en la página de datos va en blanco — el color lo dice el fondo, la etiqueta dice las palabras.
3. **Ningún traslúcido de superficie.** El «Terminar» era un rojo al 14 % de alfa: sobre la zona en verde el botón salía `#365C2A` —oliva— y su texto se quedaba en **2,37:1** (opaco sube a 5,69:1). Los tres botones son opacos, con los hexes exactos que aquel traslúcido daba sobre negro (`#240B0B` + borde `#732323`). Igual el aro «apagado» del estado sin señal, que pasa a ser el carril del bisel: un alfa calibrado contra el negro deja de significar nada en cuanto el fondo tiene color.

**El veredicto del ritmo ocupa el hueco de la ETIQUETA del segundo nivel**, no una tercera línea ni un apéndice del valor: sin objetivo la etiqueta dice «ritmo» y con objetivo dice cómo vas. Medido en el navegador, `en objetivo 4:10 /km` ocupa 180,6 de los 188 pt útiles — es la línea más apretada de la interfaz, y por eso el veredicto no puede además llevar la banda del objetivo (`4:30–4:45`). La versión con el veredicto pegado al valor medía 195 pt y se recortaba por los dos lados.

**Lo que sigue apagado a propósito:** el fondo de la página de recuperación es `restBg` (`#0D1B0F`), no un relleno de zona — en el descanso la zona deja de mandar, y es el único fondo apagado de la interfaz en marcha. Y sin ancla de FC no hay relleno ninguno: fondo negro de verdad, que es el estado de los 8 atletas de hoy.

**Desviación de proceso, declarada:** la decisión del 2026-07-28 dice que un mockup nuevo de UI de la app **nace como pantalla `propuesta` del doble**. Esta card pidió explícitamente **PNG en `docs/mocks/`, cero código** — y con razón práctica: la preview del doble vive detrás del Clerk de desarrollo y un enlace que no se abre no es una entrega. Lo que se conserva de la regla es que el PNG **no se dibuja a ojo**: `docs/mocks/tools/reloj-correr.py` calcula cada medida con las mismas constantes que hace cumplir `kit-watch/modelo.ts` y aborta si algo se sale del lienzo. Cuando esto se lleve a Swift, le toca su pantalla del doble.

**En consecuencia, no hacer:** no poner el vivo de primera página ni mover los controles de la derecha; no pintar un botón en las páginas de vivo (cada botón son 52 pt, el 21 % del lienzo, y por eso hoy el héroe se queda en 54 px); no ofrecer «Nuevo tramo» dentro de una estructura prescrita ni usarlo como segundo nombre de `Tramo hecho`; no meter la zona como fila propia de la página de datos (es lo que *significa* el pulso, va con él); no pintar zona ni relleno sin umbral; **no devolver el relleno de zona a un degradado que se va a negro, ni subirlo del 45 % (ahí muere el aro sobre ámbar); no escribir sobre el lienzo teñido con un gris fijo, con un traslúcido ni con un segundo código de color**; y no dar por bueno un ancho de la muñeca sin medirlo — el estimador del kit está calibrado para SF Pro y se queda corto con cualquier otra fuente.
