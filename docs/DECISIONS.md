...
**Queda en el defecto (a propósito o pendiente):** la cuenta atrás de carrera en `macro-progress` y `coach-ia-context` (debería ir en el día del atleta); la racha y la serie de cumplimiento del deep dive y `getDailyTssSeries` agrupan por día UTC; bordes de semana UTC en `polarization.ts` y `zones/weekly.ts`; `resolvePeriod` necesita que la app mande su día local; los crons de cuenta atrás y evaluación semanal disparan a hora UTC fija (falta hora de entrega por coach); el fallback de `runAutoPublish` y `BOX_TIMEZONE` como defecto de los formateadores.
