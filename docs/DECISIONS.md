...
**Hallazgo de token, anotado porque afecta a las nueve vistas del reloj:** `TINTE_MAX = 38 %` del kit **no es** «un tinte ambiente, oscuro y saturado apenas» (CONTRATO-UI §10.1) cuando cubre la pantalla entera — el verde de Z3 a 38 % sobre negro da `#12501E`, que es un fondo verde con letras encima y encima se parece al verde con el que se escribe el propio dato de zona, así que el color deja de discriminar. Estas láminas tiñen al **22 %**. El 38 % nunca se había mirado a tamaño real porque las nueve vistas del doble van todas sin ancla (`tinte={null}`), que es el estado real de los 8 atletas.

**El veredicto del ritmo ocupa el hueco de la ETIQUETA del segundo nivel**, no una tercera línea ni un apéndice del valor: sin objetivo la etiqueta dice «ritmo» y con objetivo dice cómo vas, con su color. Medido en el navegador, `en objetivo 4:10 /km` ocupa 180,6 de los 188 pt útiles — es la línea más apretada de la interfaz, y por eso el veredicto no puede además llevar la banda del objetivo (`4:30–4:45`). La versión con el veredicto pegado al valor medía 195 pt y se recortaba por los dos lados.
...
**En consecuencia, no hacer:** no poner el vivo de primera página ni mover los controles de la derecha; no pintar un botón en las páginas de vivo (cada botón son 52 pt, el 21 % del lienzo, y por eso hoy el héroe se queda en 54 px); no ofrecer «Nuevo tramo» dentro de una estructura prescrita ni usarlo como segundo nombre de `Tramo hecho`; no meter la zona como fila propia de la página de datos (es lo que *significa* el pulso, va con él); no pintar zona ni tinte sin umbral; y no dar por bueno un ancho de la muñeca sin medirlo — el estimador del kit está calibrado para SF Pro y se queda corto con cualquier otra fuente.
