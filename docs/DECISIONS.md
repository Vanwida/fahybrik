...
**`TBD` en `DEVELOPMENT_TEAM` es deliberado y se queda:** es lo que permite compilar contra el simulador sin cuenta de Apple, y el team id es dato del operador que no viaja en el repo (entrada del 16-ago). Lo que cambia es que ahora está **una vez** y los tres targets firmables lo heredan; divergir era un fallo que solo aparecía al archivar, con la hora de build ya gastada.
