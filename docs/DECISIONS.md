...
**El 45 % es un techo medido, no un gusto:** lo pone el **ámbar de la Z4**, porque el aro del bisel (`#FF8A4C`) tiene que mantener **3:1** contra el lienzo —es un elemento gráfico que hay que entender— y sobre ámbar al 45 % se queda en **3,09:1**; al 55 % cae a **2,30** y el aro desaparece. Con ese mismo 45 % el numeral blanco va de **7,9:1** (verde) a **12,3:1** (azul), y las cinco zonas se distinguen a un vistazo: Z1 `#3E3E40` · Z2 `#133173` · Z3 `#159438` · Z4 `#73511D` · Z5 `#732222`.

**Y de un lienzo con color se derivan tres reglas que antes no hacían falta**, porque todo se había calibrado contra el negro:

1. **El gris secundario no puede ser un hex.** `#8A8A8E` (el `dim` de WatchTheme) sobre la Z3 al 45 % se queda en **2,29:1** y la unidad y las versales dejan de leerse. El cromo pasa a ser **blanco con alfa** (0,70): mantiene ~5,5:1 contra las cinco zonas y 10,5:1 sobre negro, y sigue pesando menos que el dato (§4).
...
3. **Ningún traslúcido de superficie.** El «Terminar» era un rojo al 14 % de alfa: sobre la zona en verde el botón salía oliva y su texto se quedaba en **1,32:1**. Los tres botones son opacos, con los hexes exactos que aquel traslúcido daba sobre negro (`#240B0B` + borde `#732323`). Igual el aro «apagado» del estado sin señal, que pasa a ser el carril del bisel: un alfa calibrado contra el negro deja de significar nada en cuanto el fondo tiene color.
