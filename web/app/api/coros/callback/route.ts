...
//
// Handles the COROS OAuth 2.0 authorization-code callback. Verifies the CSRF
// state cookie to recover which athlete started the flow, exchanges the code for
// tokens, captures the COROS user id when present, and persists the connection
// (encrypted at rest) into wearable_connections via the provider-agnostic store.
//
// Mirrors /api/garmin/callback route shape and JSON error contract.
...
export async function GET(request: Request): Promise<Response> {
  ...
  // COROS surfaces user-denial / provider errors as ?error=... — surface it
  ...
  if (providerError) {
    ...
    return jsonError(400, 'coros_authorization_error', description ?? providerError);
  }
  ...
  if (!isCryptoConfigured()) {
    return jsonError(
      ...
      'ENCRYPTION_KEY env var is required to persist OAuth tokens. See /docs/coros_setup.md.',
    );
  }
  ...
  // /api/coros/connect. A missing / expired / mismatched-nonce cookie is a CSRF
  ...
  try {
    ...
    if (e instanceof OAuth2Error) {
      return jsonError(502, 'coros_token_exchange_failed', e.message);
    }
    return jsonError(502, 'coros_token_exchange_failed', (e as Error).message);
  }

  // COROS token responses may include a stable user id under one of several
  // keys. Be defensive: check the common variants in the raw body. This id is
  // what the webhook uses to resolve athlete_id (findConnectionByProviderUser).
  ...
  const tokenSet: WearableTokenSet = {
    ...
    // COROS has no scopes; persist whatever the provider echoes (likely null).
  ...
  return new Response(JSON.stringify({ ok: true, athlete_id: athlete_id.toString() }), {
    ...
    headers: {
      ...
      'set-cookie': clearStateCookie(COROS_PROVIDER, secure),
    },
  });
}
