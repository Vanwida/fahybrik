...
export default async function PublicLayout({ children, params }: Readonly<PublicLayoutProps>) {
      ...
      <div className="flex flex-1 min-h-screen flex-col bg-[color:var(--bg)] text-[color:var(--fg)]">
        <header className="sticky top-0 z-10 border-b border-[color:var(--hairline)] bg-[color:var(--bg)]/95 backdrop-blur">
          <div className="mx-auto flex h-14 w-full max-w-[840px] items-center justify-between px-6">
            ...
            >
              <Link href="/privacy" className="hover:text-[color:var(--fg)] transition-colors">
                {t('privacy')}
              </Link>
              <Link href="/terms" className="hover:text-[color:var(--fg)] transition-colors">
                {t('terms')}
              </Link>
            </nav>
          </div>
        ...
        <main className="flex-1">
          <div className="mx-auto w-full max-w-[720px] px-6 py-12 md:py-16">{children}</div>
        </main>
