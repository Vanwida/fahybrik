// v2 · BIBLIOTECA — "Codificar el método." Server component: loads the two
// library surfaces (sesiones = library blocks, microciclos = month templates) via
// the existing loaders, shapes them into the v2 view model,
...
const VALID_TABS: readonly BibliotecaTab[] = ['sesiones', 'microciclos'];
...
export default async function V2BibliotecaPage({
  ...
  const [data, progress] = await Promise.all([
    loadBibliotecaData({ coach_id: session.coach_id }).catch(() => ({
      sesiones: [],
      microciclos: [],
      counts: { sesiones: 0, microciclos: 0 },
    })),
