...
// loadSessionEditorModel) + the block library (loadLibraryRail) and hands the
// structured model to the client <SessionEditor>. A missing/unauthorized
// template degrades to an EmptyState, never 500s.

import { setRequestLocale } from 'next-intl/server';
import { getCoachSession } from '@/lib/auth/coach-session';
import {
  loadLibraryRail,
  loadSessionEditorModel,
...
export default async function V2SessionEditorPage({
  ...
  const [model, library] = await Promise.all([
    loadSessionEditorModel({ coach_id: session.coach_id, template_id: templateId }).catch(
      () => null,
    ),
  ...
  return <SessionEditor model={model} libraryBlocks={library.blocks} />;
}
