...
export function buildHoyLanes(params: {
  ...
  // the SAME level in every lane (fixes the N1/N2 cross-lane inconsistency).
  const levelByAthlete = new Map<string, AthleteLevel>();
  ...
  const espera: V2LaneCard[] = waiting
    ...
    .map((t) => ({
      ...
      // lane agrees with the roster lanes. N1 only when the athlete isn't on the
      // roster (e.g. a thread with an athlete outside the current coach filter).
      level: levelByAthlete.get(t.athlete_id) ?? ('N1' as AthleteLevel),
