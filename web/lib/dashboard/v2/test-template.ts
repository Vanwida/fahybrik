...
export function testTypeFromPrescription(p: Prescription | undefined): TestType | null {
  if (!p) return TEST_TYPES_BY_SLUG[DEFAULT_TEST_TYPE_SLUG];
  const modality = p.modality;
  if (modality !== 'run' && modality !== 'row' && modality !== 'ski') {
    return TEST_TYPES_BY_SLUG[DEFAULT_TEST_TYPE_SLUG];
  }
  // duration → total_s; distance → first set's distance meters.
  if (p.total_s !== undefined) {
    return (
      testTypeForSpec(modality, 'duration', p.total_s) ??
      closestTestType(modality, 'duration') ??
      TEST_TYPES_BY_SLUG[DEFAULT_TEST_TYPE_SLUG]
    );
  }
  ...
  if (meters !== undefined) {
    return (
      testTypeForSpec(modality, 'distance', meters) ??
      closestTestType(modality, 'distance') ??
      TEST_TYPES_BY_SLUG[DEFAULT_TEST_TYPE_SLUG]
    );
  }
