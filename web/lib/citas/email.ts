...
export async function sendBookingReceived(appt: Appt): Promise<CitaEmailResult> {
  ...
  return send({
    ...
    text:
      ...
      `Hemos recibido tu solicitud de videollamada con Pablo para el ${when} (hora de Madrid). ` +
    ...
    html: shell(
      `<h1 style="margin:8px 0 14px;font-size:22px;">Solicitud de cita recibida</h1>
       ...
       <p style="margin:0 0 12px;line-height:1.6;">Hemos recibido tu solicitud de videollamada con Pablo para el <strong>${escapeHtml(when)}</strong> (hora de Madrid). Pablo la confirmará en breve y te llegará un email con los detalles.</p>
...
export async function sendBookingInternal(appt: Appt): Promise<CitaEmailResult> {
  ...
  return send({
    ...
    text: `${name} (${appt.lead_email}) ha solicitado una videollamada para el ${when} (Madrid). Acéptala o recházala en el dashboard.`,
    html: shell(
      `<h1 style="margin:8px 0 14px;font-size:22px;">Nueva solicitud de cita</h1>
       ...
       <p style="margin:0 0 12px;line-height:1.6;">Ha solicitado una videollamada para el <strong>${escapeHtml(when)}</strong> (Madrid). Acéptala o recházala en el dashboard.</p>`,
    ),
  });
}

/** Lead: "cita confirmada" + .ics (+ Meet link if present). */
export async function sendAppointmentAccepted(appt: Appt): Promise<CitaEmailResult> {
  ...
  const ics = buildIcs({
    ...
    organizerEmail: process.env.LEADS_NOTIFY_EMAIL ?? 'hello@fahybrid.com',
...
export async function sendAppointmentCancelled(appt: Appt): Promise<CitaEmailResult> {
  ...
  return send({
    ...
    text:
      `${appt.lead_nombre ? `Hola ${appt.lead_nombre.split(' ')[0]},` : 'Hola,'}\n\n` +
      `Tu videollamada del ${when} se ha cancelado. Puedes reservar otra aquí:\n${url}\n\nEl equipo de FAHYBRID`,
    html: shell(
      `<h1 style="margin:8px 0 14px;font-size:22px;">Cita cancelada</h1>
       <p style="margin:0 0 12px;line-height:1.6;">${hi}</p>
       <p style="margin:0 0 12px;line-height:1.6;">Tu videollamada del <strong>${escapeHtml(when)}</strong> se ha cancelado. Puedes reservar otra cuando quieras:</p>
