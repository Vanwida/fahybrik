...
const reminderInputSchema = z.object({
  ...
  /** El club de esta cita — pinta su piel (nombre + acento) en el recordatorio.
   *  Null/ausente → marca de este binario, como hoy. Va como texto (id::text en SQL). */
...
export async function sendCitaReminderEmail(input: CitaReminderInput): Promise<CitaEmailResult> {
  ...
  const detailHtml = isPresencial
    ...
    : meet_link
      ? `<p style="margin:0 0 12px;"><a href="${escapeHtml(meet_link)}" style="display:inline-block;padding:12px 20px;background:${skin.light.fill};color:${skin.light.on_fill};text-decoration:none;border-radius:8px;font-weight:600;">Unirme a la videollamada</a></p>`
  ...
  const { error } = await resend.emails.send({
    ...
    html: shell(
      `<h1 style="margin:8px 0 14px;font-size:22px;">Mañana a las ${escapeHtml(time)}</h1>
       ...
       ${detailHtml}
       <p style="margin:24px 0 0;color:#666;">¡Nos vemos! · El equipo de ${escapeHtml(skin.wordmark)}</p>`,
      skin,
    ),
  });
