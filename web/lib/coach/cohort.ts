...
async function rollupAthlete(
  ...
  const days_to_a_event = a_event_iso ? daysBetweenIso(todayIso, a_event_iso) : null;
  ...
  const next_session: CohortRow['next_session'] =
    a.next_session_iso == null
      ? null
      : { label: nextSessionLabel(a.next_session_iso, todayIso), iso_date: a.next_session_iso };
