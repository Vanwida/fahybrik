...
import { addDays, isoDateString, parseIsoDate, startOfDayUtc } from './dates';
...
export async function computeMacrocycle(params: {
  ...
  if (eventDate < startOfDayUtc(new Date())) {
...
export async function getNextWorkout(params: {
  ...
  const client = params.client ?? defaultSql;
  const todayIso = isoDateString(startOfDayUtc(params.on_date ?? new Date()));
