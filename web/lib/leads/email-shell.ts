...
/** The orange CTA button (escapes href + label). */
export function ctaButton(href: string, label: string): string {
  return `<p style="margin:4px 0 20px;"><a href="${escapeHtml(href)}" style="display:inline-block;padding:12px 20px;background:${BRAND_ORANGE};color:${BRAND_INK};text-decoration:none;border-radius:8px;font-weight:600;">${escapeHtml(label)}</a></p>`;
}
